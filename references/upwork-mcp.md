...
- **A freelancer cannot message a client first on a proposal.** No room exists until the client writes. Applied proposals create no follow-up or task. `/sync` moves one to `lost` after 14 full days without a client reply.
