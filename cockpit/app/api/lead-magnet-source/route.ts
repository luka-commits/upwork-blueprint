...
export async function POST(req: Request) {
  ...
  if (!ID.test(id) || !website || !location) return json({ error: 'A job, business website and location are required.' }, 400);
  if (website.length > 500 || location.length > 160 || placeId.length > 220) return json({ error: 'The lead magnet source is too long.' }, 400);
  const args = ['lead-magnet-source', id, website, '--location', location, '--language', language];
