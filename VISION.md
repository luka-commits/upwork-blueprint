...
Loom, application, conversation, follow-up, proposal and delivery.
...
They record the Loom, review client-facing copy and submit every proposal on
Upwork. A reply can be sent from the cockpit only after the member approves that
exact text with one click.
...
- Every client-facing claim is supported, every send is intentional and every
  run leaves a clear next action.

## Review contract

Review the system against this file, then the evidence in tests and the real
interface. A review ends PASS or FAIL and answers five questions:
