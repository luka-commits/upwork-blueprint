...
It never sends anything to a client without your yes, never buys Connects, and never runs on its own in the background. Upwork suspends accounts for unattended automation, so every search and every send starts with you. The details are in [references/upwork-rules.md](references/upwork-rules.md).
